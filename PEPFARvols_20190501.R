...
# combine funded Volunteers with VIDA Geoterms data, (full outer join - includes PEPFAR & non-PEPFAR Funded PCVs)
combineddf <- merge(dfunding, dvida, by=c("Post", "VolID"), all = TRUE)

write.csv(combineddf,"CombinedPEPFARvols_20190501.csv", row.names = FALSE)
